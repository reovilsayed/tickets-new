...
class TOCOnlineService
{
    ...
    public function createMagazineCustomer(MagazineOrder $order)
    {
    ...
    public function getOrCreateMagazineCustomer(MagazineOrder $order): Customer
    {
        ...
        $res = $this->createMagazineCustomer($order);
    ...
    public function createMagazineCommercialSalesDocument(MagazineOrder $magazineOrder)
    {

        $token = $this->getAccessTokenFromRefreshToken();

        $lines = [];
        if ($magazineOrder->items->count()) {
        ...
        $body = [
            ...
            'date' => Carbon::now()->format('Y-m-d'),
            'finalize' => 0,
            'customer_id' => $magazineOrder->billing?->vatNumber ? $this->getOrCreateMagazineCustomer($magazineOrder)->customer_id : 452,
            'customer_tax_registration_number' => $magazineOrder->billing?->vatNumber ? $magazineOrder->billing->vatNumber : null,
            'customer_business_name' => $magazineOrder->billing?->vatNumber ? @$magazineOrder->billing?->name : null,
            'customer_address_detail' => $magazineOrder->billing?->vatNumber ? @$magazineOrder->billing?->address : null,
            'customer_postcode' => '',
            'customer_city' => '',
            'customer_country' => 'PT',
            'due_date' => Carbon::now()->addDays(5)->format('Y-m-d'),
    ...
    // this method need to call when payment completed
    public function sendMagazineEmailDocument(Order $order, $invoice_id)
    {
        $token = $this->getAccessTokenFromRefreshToken();
