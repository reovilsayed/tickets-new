    ...
    <div class="email-container">
        ...
        <div class="email-content">
            <div style="text-align: right">
                ...
                <p>{{ __('words.invite_thank_you_for_purchasing_tickets_to') }} </p>

                <p><strong>{{ __('words.invite_event_name') }}:</strong> {{ $product->event->name }} </p>
                <p><strong>{{ __('words.invite_ticket_name') }}:</strong> {{ $product->name }} </p>
                <p><strong>{{ __('words.date') }}:</strong> {{ $product->start_date->format('d M, Y') }} -
                    {{ $product->end_date->format('d M, Y') }}</p>
                <p><strong>{{ __('words.invite_doors_open') }}:</strong> {{ $product->event->start_at->format('h:i a') }}</p>
                <p><strong>{{ __('words.invite_location') }}:</strong>{{ $product->event->location }}</p>
            </div>
        </div>
        <div class="" style="text-align: center">
            <a href="{{ route('download.ticket', ['order' => $order, 'p' => $product->id, 't' => @$ticket?->ticket]) }}"
                class="btn-download text-white">{{ __('words.download_invite_ticket') }}</a>
        </div>
