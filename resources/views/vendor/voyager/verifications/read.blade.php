...
<h1 class="page-title">
    ...
    @endcan
    @if($dataTypeContent->user->shop)
    <a class="btn btn-sm btn-dark pull-right view " style="margin-left: 5px;" href="{{route('admin.shop.active',$dataTypeContent->user->shop)}}"><span class="hidden-xs hidden-sm " style="margin-left: 5px;">{{$dataTypeContent->user->shop->status==0 ?  'Active' :'Deactive' }}</span></a>
    @else
    <h1 style="background-color:red;color:#fff;text-align:center;font-size:30px" >This user has not created a shop.</p>
    @endif
</h1>
